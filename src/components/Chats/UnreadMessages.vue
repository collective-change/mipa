        ...
        <q-list style="min-width: 250px; max-width: 350px">
          <div v-for="chat in unreadChats" :key="chat.id">
            <q-item
              clickable
              v-close-popup
              @click="$router.push(getRouteFromChat(chat))"
            >
              <q-item-section>
                <q-item-label>
                ...
                <q-item-label caption
                  >{{
                    chat.newestMessages[chat.newestMessages.length - 1]
                      .fromNameCached
                  }}:
                  {{
                    chat.newestMessages[chat.newestMessages.length - 1].text
                  }}</q-item-label
                >
              </q-item-section>
          ...
          routeObj = {
            ...
            params: {
              orgNameSlug: chat.orgNameSlugCached
                ...
                : "-",
              orgId: chat.orgId,
              modelId: chat.subjectDocLineage.modelId,
              nodeId: chat.subjectDocLineage.nodeId,
            },
          };
          break;
        case "action":
