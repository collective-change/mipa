...
function getNodeLinkEndPoints(d, radius, targetOffset) {
  ...
  if ("hasReciprocal" in d && d.hasReciprocal == true) {
    var angleOffset = 0.3;
  } else {
    var angleOffset = 0;
  }

  var t_gamma = Math.atan2(dy, dx) - angleOffset; // Math.atan2 returns the angle in the correct quadrant as opposed to Math.atan
  var tx = d.target.x - Math.cos(t_gamma) * (radius + targetOffset);
  var ty = d.target.y - Math.sin(t_gamma) * (radius + targetOffset);

  var s_gamma = Math.atan2(-dy, -dx) + angleOffset;
